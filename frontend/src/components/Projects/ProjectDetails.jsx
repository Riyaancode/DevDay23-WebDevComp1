...
const organisation = [
  { name: "GeeksofKolxsxachi" },
  { name: "CodersofKolachi" },
  { name: "TechofKolachi" },
];

export default function ProjectDetails() {
  const { name, organisationName } = useLocation().state;
  ...
  const [activeNav, setActiveNav] = useState({
    label: "Overview",
    children: <Overview name={name} />,
  });
  const Nav = ({ label, onClick, children }) => {
    return (
      <button
        className={`flex items-center gap-2 px-4 py-2 w-full hover:bg-gray-100 hover:text-blue-700 ${
          activeNav.label == label
      ...
      <nav className="bg-white border-b">
        <div className="flex items-center space-x-8 py-3 px-4 max-w-screen-xl mx-auto md:px-8">
          <div className="flex align-bottom justify-items-center">
            ...
            <nav className="flex ml-3 align-middle" aria-label="Breadcrumb">
              <ol className="inline-flex items-center align-middle space-x-1 md:space-x-3">
                <li>
        ...
        <div className="flex h-screen flex-col hidden lg:block  justify-between border-r bg-white">
          <div className=" w-52 py-3">
            ...
            >
              <Navs
                tabs={[
                  { label: "Overview", children: <Overview name={name} /> },
                  {
                    label: "Work items",
                    children: <WorkItems />,
                  },
                  {
                    label: "Backlogs",
                    children: <div>This is the Backlogs tab.</div>,
                  },
                ]}
              />
            </nav>
